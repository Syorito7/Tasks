...
int main()
{
	try
	{
		...
		linalg::Matrix<> QQ{ {1,1},{1,2},{1,3} };

		linalg::Matrix<> vector_TMP{ 1,2,3 };
		Complex<> a1(1, 0), a2(2, 2), a3(3, -1.5), a4(4, 0);

		linalg::Matrix<Complex<>> Matrix_with_complex_value{ {a1,a2} , {a3,a4} };
		std::cout << "Matrix with complex values :\n" << Matrix_with_complex_value;
		
		// Так как в выводе комплексного числа был убран "\n", то в тестах его надо добавить 
		...
		//std::cout << "Determinat = " << QQ.det() << '\n';
		std::cout << "Complex determinat = " << Matrix_with_complex_value.det() << '\n';
		// Отлично, работает
		...
		// Нужно перегрузить оператор / - для комплексного числа - ОК

		// Также надо учесть fabs для комлексного числа - НЕ ОК!
		// Так как нет понятие комплексное число больше или меньше другого, то скипаем этот момент
		//std::cout << "Complex Rank = " << Matrix_with_complex_value.rank() << '\n';

		// ОК
		double n = matr.norm();
		std::cout << "Norm = " << n << '\n';
		std::cout << "Norm = " << TMP.norm() << '\n';
		// не понимает - как взять корень из комплексного числа, так как в норме используется sqrtf()
		// Также не понимает, что норма только вещественная
		//std::cout << "Complex Norm = " << Matrix_with_complex_value.norm() << '\n';
		...
		// Метод trace возвращает double, поэтому ругается - надо исправить на шаблонный тип
		//std::cout << "Complex trace = " << Matrix_with_complex_value.trace() << '\n';


		// ОК - и для комплексной матрицы
		linalg::Matrix<> m_tr = linalg::transpose(matr);
		linalg::Matrix<> m_TMP = linalg::transpose(TMP);
		std::cout << "Transpose for vector TMP :\n" << linalg::transpose(vector_TMP);
		std::cout << "m_tr" << '\n' << m_tr;
		std::cout << "m_TMP" << "\n" << m_TMP;
		std::cout << "Complex transpose :\n" << linalg::transpose(Matrix_with_complex_value);

		// Обратная матрица - надо проверить
		
		// Не работает с комплексными числами из-за fabs из-за rank
		// Вариант решение - взять модуль комплескного числа
		//std::cout << "Inverse Complex Matrix :\n" << linalg::inv(Matrix_with_complex_value)
		...
		// Проверка того, что умножение исходной матрицы на обратную - даёт единицну
		std::cout << "A * A-inverse :\n";
		std::cout << A_for_inverse * linalg::inv(A_for_inverse);

		// Нужно проверить на реализацию с комплексными числами
		...
		std::cout << "m_p" << '\n' << m_p;
		std::cout << "pow_TMP" << '\n' << pow_TMP;
		// Комплексная матрица - ОК
		std::cout << "Pow 2 Complex Matrix :\n" << linalg::pow(Matrix_with_complex_value,2);
		...
		// Здесь надо посмотреть на работу исключений, начиная с angle
		...
		*/
		// Угол между матрицей и вектором. Проверка, что его нет.
		//std::cout << "Angle between TMP and Vector_TMP :\n" << linalg::angle(TMP, vector_TMP);
		...
		// Здесь надо сделать проверку для матрицы
		...
		std::cout << "Unit_vector = " << '\n' << TMP_vectorik;
		// Проверка для нормировки матрицы, которой нет
		//std::cout << "Unit_Matrix = " << '\n' << linalg::unit(TMP);
		...
		// 1.1 - Обратная матрица для прямоугольной T - ОК

		/*
		linalg::Matrix<> T{ {1,2,3},{5,4,3} };
		std::cout << "Matrix T :\n" << T;
		std::cout << "Inverse T :\n" << linalg::inv(T);

		// 1.2 - Определитель
		// Срабатывание исключения для прямоугольной матрицы 
		...
		*/
		...
		// проверка деления комплексного числа на комплексное - Ок
		std::cout << "(2+4i) / (-1+3i) = " << Complex<>(2, 4) / Complex<>(-1, 3) << '\n';
		// ОК
		val += Complex<int>{5, 7};
